//
//  RatingDisplayViewModelDeprecatedTests.swift
...
final class RatingDisplayViewModelDeprecatedTests: XCTestCase {

    // MARK: - Properties
    var theme: ThemeGeneratedMock!
    var colorsUseCase: RatingGetColorsUseCaseDeprecatedableGeneratedMock!
    var sizeUseCase: RatingSizeAttributesUseCaseDeprecatedableGeneratedMock!
    var sut: RatingDisplayViewModelDeprecated!
    var subscriptions: Set<AnyCancellable>!

    // MARK: - Setup
    override func setUp() {
        super.setUp()
        self.colorsUseCase = RatingGetColorsUseCaseDeprecatedableGeneratedMock()
        self.sizeUseCase = RatingSizeAttributesUseCaseDeprecatedableGeneratedMock()
        self.theme = ThemeGeneratedMock.mocked()
        self.subscriptions = .init()

        self.colorsUseCase.executeWithThemeAndIntentAndStateReturnValue = RatingColorsDeprecated(
            fillColor: self.theme.colors.main.onMain,
            strokeColor: self.theme.colors.base.background,
            opacity: 1.0)

        self.sizeUseCase.executeWithSpacingAndSizeReturnValue = RatingSizeAttributesDeprecated(borderWidth: 2.0, height: 20, spacing: 8)

        self.sut = RatingDisplayViewModelDeprecated(
            theme: self.theme,
            intent: .main,
            size: .small,
            count: .five,
    ...
    func test_theme_updated() {
        ...
        // Given
        let colorsPublisherMock: PublisherMock<Published< RatingColorsDeprecated>.Publisher> = .init(publisher: self.sut.$colors)
        let ratingSizePublisherMock: PublisherMock<Published<RatingSizeAttributesDeprecated>.Publisher> = .init(publisher: self.sut.$ratingSize)
